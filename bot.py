...
# Minimum tasdiqlash soni (5 dan kam bo'lsa signal berilmaydi)
MIN_CONFIRMATIONS = 5

SYMBOLS = [
    {"symbol": "XAU/USD", "name": "Oltin",       "type": "forex",  "pip": 0.10},
    {"symbol": "EUR/USD", "name": "Euro/Dollar",  "type": "forex",  "pip": 0.0001},
    {"symbol": "GBP/USD", "name": "Funt/Dollar",  "type": "forex",  "pip": 0.0001},
    {"symbol": "GBP/JPY", "name": "Funt/Yen",     "type": "forex",  "pip": 0.01},
    {"symbol": "BTC/USD", "name": "Bitcoin",      "type": "crypto", "pip": 10},
    {"symbol": "ETH/USD", "name": "Ethereum",     "type": "crypto", "pip": 1},
    {"symbol": "AAPL",    "name": "Apple",        "type": "stock",  "pip": 0.01},
    {"symbol": "TSLA",    "name": "Tesla",        "type": "stock",  "pip": 0.01},
    {"symbol": "SPY",     "name": "S&P500 ETF",   "type": "stock",  "pip": 0.01},
    {"symbol": "NVDA",    "name": "Nvidia",       "type": "stock",  "pip": 0.01},
    {"symbol": "US30",    "name": "Dow Jones",    "type": "index",  "pip": 1.0},
]
...
def analyze_symbol(item: dict) -> dict | None:
    ...
    if len(action_confs) < MIN_CONFIRMATIONS or score < 6:
        print(f"  {symbol}: zaif signal ({len(action_confs)} tасdiqlаsh, score={score}) — o'tkazib yuborildi")
        return None
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"Natija: {len(signals)} ta kuchli signal topildi")
    print(f"{'='*50}")

    if signals:
        # Eng kuchli signallarni oldin yuborish
        signals.sort(key=lambda x: x["score"], reverse=True)
        header = (
            f"📡 <b>KUNLIK TAHLIL — {datetime.utcnow().strftime('%d.%m.%Y')}</b>\n"
            f"Kuchli signal: <b>{len(signals)} ta</b>\n"
            f"{'─'*28}\n"
            f"Faqat 5+ tasdiqlangan signallar ko'rsatilmoqda."
        )
        ...
        send_telegram(
            f"📭 <b>Kunlik tahlil — {datetime.utcnow().strftime('%d.%m.%Y')}</b>\n\n"
            f"Bugun hech qanday <b>kuchli</b> signal topilmadi.\n"
            f"Barcha juftliklar tekshirildi — shartlar bajarilmadi.\n\n"
            f"<i>Bu yaxshi belgi — bozor noaniq. Kutish strategiyasi eng to'g'ri.</i>"
        )
